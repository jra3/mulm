...
test.describe("Plant Submissions", () => {
	test.beforeEach(async () => {
		await cleanupTestUserSubmissions(TEST_USER.email);
	});

	test("should show Plant-specific fields (propagation method, lighting) and hide Fish fields", async ({ page }) => {
		await login(page);
		await page.goto("/submissions/new");
		await page.waitForSelector("#bapForm");

		// Select Plant species type
		...
		// Plant-specific fields should be visible
		await expect(page.locator('input[name="propagation_method"]')).toBeVisible();
		...
		await expect(page.locator('select[name="foods"]')).not.toBeVisible();
	...
	test("should allow saving a draft with partial Plant data", async ({ page }) => {
		...
		await page.selectOption('select[name="species_class"]', "Cryptocoryne");
		...
		await fillTomSelectTypeahead(page, "species_common_name", "Wendt's Cryptocoryne");
		await fillTomSelectTypeahead(page, "species_latin_name", "Cryptocoryne wendtii");

		// Don't fill propagation_method or lighting fields - partial draft
		...
		try {
			...
			expect(submissions[0].species_type).toBe("Plant");
			expect(submissions[0].species_class).toBe("Cryptocoryne");
		} finally {
			await db.close();
		}
	});

	test("should require propagation_method for Plant submission", async ({ page }) => {
		await login(page);
		await page.goto("/submissions/new");
		await page.waitForSelector("#bapForm");

		// Fill all required fields EXCEPT propagation_method
		await page.selectOption('select[name="water_type"]', "Fresh");
		await page.selectOption('select[name="species_type"]', "Plant");
		await page.waitForLoadState("networkidle");
		await page.waitForSelector('select[name="species_class"]', { state: "visible" });
		...
		await page.fill('input[name="light_type"]', "LED");
		await page.fill('input[name="light_strength"]', "200W");
		await page.fill('input[name="light_hours"]', "16");
		...
		await page.fill('input[name="substrate_type"]', "Soil");
		await page.fill('input[name="substrate_depth"]', "2 inches");
		await page.fill('input[name="substrate_color"]', "Dark");

		// Leave propagation_method empty - should fail validation
	...
	test("should require all lighting fields for Plant submission", async ({ page }) => {
		await login(page);
		await page.goto("/submissions/new");
		await page.waitForSelector("#bapForm");
		...
		// Fill propagation method
		await page.fill('input[name="propagation_method"]', "Cuttings");

		// Fill only 2 of 3 lighting fields - missing light_hours
		await page.fill('input[name="light_type"]', "LED");
		await page.fill('input[name="light_strength"]', "200W");
		// Leave light_hours empty
	...
	test("should successfully submit complete Plant form", async ({ page }) => {
		...
		// Fill Plant-specific required fields
		await page.fill('input[name="propagation_method"]', "Cuttings");
		await page.fill('input[name="light_type"]', "LED");
		await page.fill('input[name="light_strength"]', "200W");
		await page.fill('input[name="light_hours"]', "16");
		...
		const errors = await page.locator('.error-message, [aria-invalid="true"]').count();
		console.log(`Found ${errors} validation errors before submit`);
		if (errors > 0) {
			const errorTexts = await page.locator('.error-message').allTextContents();
		...
		try {
			...
			// Verify it's a Plant submission
			expect(submission.species_type).toBe("Plant");
			expect(submission.program).toBe("plant");
			expect(submission.propagation_method).toBe("Cuttings");
			expect(submission.light_type).toBe("LED");
			expect(submission.light_strength).toBe("200W");
			expect(submission.light_hours).toBe("16");
