...
function generateCatfishSubmission(
  ...
  return {
    ...
    substrate_color: randomChoice(["natural", "black"]),
    foods: [
      randomChoice(["sinking pellets", "bloodworms", "algae wafers"]),
      randomChoice(fishFoods),
    ],
  ...
  try {
    ...
    for (let i = 0; i < userNames.length; i++) {
      ...
      if (existingUser) {
        ...
        userId = await createMember(email, name, { password }, false);
    ...
    for (const user of users) {
      ...
      if (user.name === "John Allen") {
        logger.info(
          "Creating catfish submissions for John Allen to earn Catfish Specialist award..."
        );
        ...
        for (const catfish of catfishNames) {
          ...
          logger.info(
            `Created catfish submission ${submissionId} for John Allen: ${catfish.common}`
          );
        }
        ...
        logger.info(
          "Creating Anabantoids submissions for Sarah Johnson to earn Anabantoids Specialist award..."
        );
        ...
        for (const anabantoid of anabantoidNames) {
          ...
          logger.info(
            `Created Anabantoids submission ${submissionId} for Sarah Johnson: ${anabantoid.common}`
          );
        }
      } else if (user.name === "Michael Chen") {
        // Michael Chen gets Livebearers submissions for specialty award
        logger.info(
          "Creating Livebearers submissions for Michael Chen to earn Livebearers Specialist award..."
        );
        ...
        for (const livebearer of livebearerNames) {
          ...
          logger.info(
            `Created Livebearers submission ${submissionId} for Michael Chen: ${livebearer.common}`
          );
        }
      } else {
        ...
        for (let i = 0; i < numSubmissions; i++) {
          ...
          logger.info(
            `Created ${submissionType} submission ${submissionId} for user ${user.name} (submitted: ${shouldSubmit})`
          );
        }
      }
    }
    ...
    for (const submission of submissionsToApprove) {
      try {
        ...
        if (Math.random() > 0.15) {
          ...
          logger.info(
            `Recorded species: ${submission.latinName} (${submission.commonName}) with ID: ${speciesNameId}`
          );
