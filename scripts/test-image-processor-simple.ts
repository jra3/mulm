...
async function runTests() {
  ...
  console.log(
    `✓ Large image processed: ${largeResult.metadata.originalWidth}x${largeResult.metadata.originalHeight}`
  );
  ...
  console.log(
    `  • Savings: ${Math.round((1 - webpResult.original.size / result.original.size) * 100)}%`
  );
