...
function inferProgramClass(tags: string, commonName: string): string {
  ...
  if (
    ...
    nameLower.includes("rotala") ||
    nameLower.includes("ludwigia") ||
    nameLower.includes("hygrophila")
  )
    return "Stem Plants";
...
async function main() {
  ...
  for (const species of sortedSpecies) {
    ...
    const externalRefsJson =
      species.externalReferences.size > 0
  ...
  for (const [programClass, count] of Array.from(classDistribution.entries()).sort(
    (a, b) => b[1] - a[1]
  )) {
