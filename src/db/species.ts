...
/**
 * Get all approved submissions for a species group, showing sync status
 */
export async function getSubmissionsForSpecies(groupId: number) {
  try {
    const submissions = await query<{
      ...
      LEFT JOIN species_common_name scn ON s.common_name_id = scn.common_name_id
      LEFT JOIN species_scientific_name ssn ON s.scientific_name_id = ssn.scientific_name_id
      JOIN species_name_group sng ON (scn.group_id = sng.group_id OR ssn.group_id = sng.group_id)
      WHERE sng.group_id = ?
        AND s.approved_on IS NOT NULL
    ...
    throw new Error("Failed to get submissions for species");
  }
}

/**
 * Get submission sync statistics for a species group
 */
export async function getSubmissionSyncStats(groupId: number) {
  try {
    const stats = await query<{
      total_submissions: number;
      synced_submissions: number;
      unsynced_submissions: number;
    }>(
      `
      SELECT
        COUNT(*) AS total_submissions,
        SUM(CASE WHEN s.species_class = sng.program_class THEN 1 ELSE 0 END) AS synced_submissions,
        SUM(CASE WHEN s.species_class != sng.program_class THEN 1 ELSE 0 END) AS unsynced_submissions
      FROM submissions s
      LEFT JOIN species_common_name scn ON s.common_name_id = scn.common_name_id
      LEFT JOIN species_scientific_name ssn ON s.scientific_name_id = ssn.scientific_name_id
      JOIN species_name_group sng ON (scn.group_id = sng.group_id OR ssn.group_id = sng.group_id)
    ...
    return stats[0] || { total_submissions: 0, synced_submissions: 0, unsynced_submissions: 0 };
  } catch (err) {
    logger.error(`Failed to get submission sync stats for species ${groupId}`, err);
    throw new Error("Failed to get submission sync stats");
  }
}

/**
 * Sync all submissions for a species to match the species group's program_class
 */
export async function syncSubmissionsForSpecies(groupId: number) {
  try {
    // Get the canonical program_class from species_name_group
    const speciesGroup = await query<{ program_class: string }>(
      "SELECT program_class FROM species_name_group WHERE group_id = ?",
      [groupId]
    );
    ...
    const correctClass = speciesGroup[0].program_class;

    // Update all approved submissions linked to this species (via common or scientific names)
    const result = await writeConn.run(
      `
      UPDATE submissions
      SET species_class = ?
      WHERE id IN (
        ...
        LEFT JOIN species_common_name scn ON s.common_name_id = scn.common_name_id
        LEFT JOIN species_scientific_name ssn ON s.scientific_name_id = ssn.scientific_name_id
        WHERE (scn.group_id = ? OR ssn.group_id = ?)
          AND s.approved_on IS NOT NULL
          AND s.species_class != ?
      )
    `,
      [correctClass, groupId, groupId, correctClass]
    );

    logger.info(
      `Synced ${result.changes} submissions for species group ${groupId} to class "${correctClass}"`
    );
