...
| `lookup.ts` | `findSpeciesById`, `findSpeciesByIds`, `findSpeciesByName` (the Species holding a Name, by text and optionally kind), and `resolveSpecies`: a pair of spellings to one Species (Latin as a scientific Name, the Canonical name among them, then common as a common Name). Imports bind by it. |
