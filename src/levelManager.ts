...
export async function checkAndUpdateMemberLevel(
  ...
  try {
    ...
    if (calculatedLevel !== currentLevel) {
      logger.info(
        `Level upgrade detected for member ${memberId}: ${currentLevel} → ${calculatedLevel} (${program})`
      );
...
export async function checkAllMemberLevels(
  ...
  const results: Record<string, LevelCheckResult & { error?: boolean }> = {};

  for (const program of ["fish", "plant", "coral"] as Program[]) {
    try {
      results[program] = await checkAndUpdateMemberLevel(memberId, program, options);
    } catch (error) {
