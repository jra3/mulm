 ...
 */
router.post('/image', upload.array('images', 5), async (req: MulmRequest, res): Promise<void> => {
  const { viewer } = req;
 ...
 */
router.get('/progress/:uploadId', (req: MulmRequest, res) => {
 ...
 */
router.delete('/image/:key', async (req: MulmRequest, res): Promise<void> => {
  const { viewer } = req;
