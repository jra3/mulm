...
export const ApiErrors = {
  SEARCH_FAILED: (entity: string) =>
    createApiError(`Unable to search ${entity}`, `${entity.toUpperCase()}_SEARCH_ERROR`),
  NOT_FOUND: (entity: string) =>
    createApiError(`${entity} not found`, `${entity.toUpperCase()}_NOT_FOUND`),
  VALIDATION_FAILED: (message?: string) =>
    createApiError(message || "Validation failed", "VALIDATION_ERROR"),
...
export const sendApiErrors = {
  searchFailed: (res: Response, entity: string) =>
    sendApiError(res, 500, `Unable to search ${entity}`, `${entity.toUpperCase()}_SEARCH_ERROR`),
  notFound: (res: Response, entity: string) =>
    sendApiError(res, 404, `${entity} not found`, `${entity.toUpperCase()}_NOT_FOUND`),
  validationFailed: (res: Response, message?: string) =>
    sendApiError(res, 400, message || "Validation failed", "VALIDATION_ERROR"),
